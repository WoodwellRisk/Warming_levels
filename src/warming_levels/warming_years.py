...
def get_cmip6_at_warming_level(mms, cmip6_variable, warming_level, outfn, out_temporal_res, max_year=2100, warming_year_type='moving_window', year_window=21, temp_window=0.5):
    ...
    # in testing, chunk warnings were thrown on xr.merge([xout,dr_out]) when eleven or more model/member/scenario combinations were requested at once.
    ...
    for r in range(iter_tab.shape[0]):
        ...
        dr = dat[cmip6_variable]
        ds_out = xr.Dataset({"lat":(["lat"], newlats,
                                    {"units":"degrees_north"}),
                             "lon":(["lon"], newlons,
                                    {"units":"degrees_east"})})
        regridder = xe.Regridder(dat, ds_out, "bilinear")
        dr_out = regridder(dr, keep_attrs=True)
        # keep the global attributes
        oldattrs = dat.attrs
        oldattrs.update(dr_out.attrs)
        dr_out.attrs = oldattrs

        # remove lat and lon bounds since not all datasets have these 
        # variables, and we changed the lat and lon anyway
        dr_out = dr_out.drop(['lat_bnds','lon_bnds','bnds'],errors='ignore')
        if r==0:
            xout = dr_out
        else:
            xout = xr.merge([xout,dr_out])
